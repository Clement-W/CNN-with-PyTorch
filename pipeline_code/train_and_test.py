...
def test(model, test_loader):
    ...
    progressB = tqdm.tqdm(enumerate(test_loader),total=len(test_loader),position=0, leave=True)
...
def train(model, train_loader, test_loader ,criterion, optimizer, config):
    ...
    for epoch in range(1,nb_epochs+1):
        ...
        progressB = tqdm.tqdm(enumerate(train_loader),total=len(train_loader),position=0, leave=True)
