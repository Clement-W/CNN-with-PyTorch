...
def train(model, train_loader, test_loader, criterion, optimizer, config):
    ...
    for epoch in range(1, nb_epochs + 1):
        ...
        )  # tqdm iterator -> show a progress bar

        totalCorrect = (
            0  # Keep the number of correct answers to compute the training accuracy
        )
        total = 0
...
if __name__ == "__main__":
    ...
    config = dict(
        ...
        kernels=[
            ...
            32,
            64,
        ],  # Number of kernels in each convolution layer (16 kernels in the first conv layer in this example)
        batch_size=128,
        learning_rate=0.001,
        dataset="MNIST",
        architecture="CNN",
